namespace L11_Snowboardpiste_No4 {
    ...
    function handleLoad(_event: Event): void {
        ...
        house = new House();
        lift = new Lift(new Vector(200,-44),new Vector(-500,150),"lift");
        mountain = new Mountain();
        snowboarder = new Snowboarder(new Vector(0, 250), new Vector(crc2.canvas.width, 0),"snowboarder");
        piste = new Piste();
        sun = new Sun();
        tree = new Tree();
        golden = 0.62;
    ...
    function createSnowflakes(nFlakes: number) {
        for (let i: number = 0; i < nFlakes; i++) {
            ...
            let snowflake: Snow = new Snow(new Vector(x, y), new Vector(600, 0),"snow");
    ...
    function createSnowboarder(nSnowboarder: number): void {
        for (let i: number = 0; i < nSnowboarder; i++) {
            ...
            let snowboarder: Snowboarder = new Snowboarder(new Vector(-200, 0), new Vector(x, y),"snowboarder");
